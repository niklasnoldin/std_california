<template>
  <div>
    <div id="map" @mouseout="current = null"></div>
    <div class="info">
    ...
    >
      ...
      <h4>
        {{ ((current.perMil / 1000000) * 100) | decimal(1) }} cases
        <span class="footnote">- per 100 residents</span>
      </h4>

      <p v-if="!highlightedDisease || highlightedDisease === ''">Total cases: {{ current.total | decimal }}</p>
      <p v-if="highlightedDisease === 'Early Syphilis'">Total Early Syphilis cases: {{ current.syph | decimal }}</p>
      <p v-if="highlightedDisease === 'Gonorrhea'">Total Gonorrhea cases: {{ current.gon | decimal }}</p>
      <p v-if="highlightedDisease === 'Chlamydia'">Total Chlamydia cases: {{ current.clam | decimal }}</p>

      <ul v-if="!highlightedDisease || highlightedDisease === ''">
        <li>Early Syphilis cases: {{ current.syph | decimal }}</li>
...
export default {
  ...
  computed: {
    ...
    casesPer1000() {
      ...
      for (const county of this.counties) {
        const filteredData = this.data.filter(
          (row) => row.County === county && row.Sex === "Total" && (!this.highlightedDisease || this.highlightedDisease === row.Disease)
        );
    ...
    colorFull() {
      if (this.highlightedDisease === 'Chlamydia')
        return '#f09d51';
      if (this.highlightedDisease === 'Gonorrhea')
        return '#de74a5';
      if (this.highlightedDisease === 'Early Syphilis')
        return '#029da9';
      return '#4f1d79'
    },
     colorEmpty() {
      ...
      this.svg
        ...
        .attr("fill", (d) => {
          ...
          return d3
              ...
              .range([this.colorEmpty, this.colorFull])(casesPer1000)
        })
        .attr("stroke", "white")
